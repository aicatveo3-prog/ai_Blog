...
상하이의 MiniMax가 2026년 6월 1일 **MiniMax M3**를 출시했다(API 즉시 제공, 오픈 가중치는 6월 7일 Hugging Face 공개). **오픈웨이트 최초로 프런티어급 코딩·에이전트 성능 + 1M 토큰 컨텍스트 + 네이티브 멀티모달을 한 아키텍처에 결합**했다고 밝혔다.
...
## 가격 · 라이선스
- 가격: 입력 **$0.30/M**, 출력 **$1.20/M**. 프롬프트 캐싱 지원(반복 컨텍스트 최대 90% 절감).
- 라이선스: **MIT** — 상업적 사용·셀프호스트 가능.
